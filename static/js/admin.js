...
window.loginAdmin = async function() {
  ...
  try {
    ...
    console.warn('[Admin] Server login error, attempting local credential check:', err);
  }

  // Fallback: Only allow login if credentials match hardcoded admin credentials
  if (email === ADMIN_CREDENTIALS.email.toLowerCase() && pass === ADMIN_CREDENTIALS.password) {
    const token = 'ga_admin_token_' + btoa(email + ':' + Date.now());
    sessionStorage.setItem('genius_admin_session', 'active');
    localStorage.setItem('genius_admin_token', token);
    localStorage.setItem('genius_current_admin', JSON.stringify({ email: email, token: token }));
    showDashboard();
  } else {
    if (errorEl) {
...
async function refreshAllData(skipSync = false) {
  ...
  // PRIMARY: Fetch data directly from the server (single source of truth)
  try {
    const fetchFn = window.geniusFetch || fetch;
    const res = await fetchFn('/api/cloud-sync?cb=' + Date.now());
    if (res && res.ok) {
      serverData = await res.json();
      if (serverData && typeof serverData === 'object') {
        approved = serverData.geniusact_approved_users || [];
        pending = serverData.geniusact_pending_users || [];
        visitors = serverData.geniusact_visitor_logs || [];
        // Update localStorage cache so other components stay in sync
        ...
        if (serverData.geniusact_support_messages) localStorage.setItem('geniusact_support_messages', JSON.stringify(serverData.geniusact_support_messages));
        if (serverData.geniusact_contact_chats) localStorage.setItem('geniusact_contact_chats', JSON.stringify(serverData.geniusact_contact_chats));
        if (serverData.geniusact_bank_links) localStorage.setItem('geniusact_bank_links', JSON.stringify(serverData.geniusact_bank_links));
        if (serverData.geniusact_withdrawal_requests) localStorage.setItem('geniusact_withdrawal_requests', JSON.stringify(serverData.geniusact_withdrawal_requests));
        if (serverData.geniusact_user_footprints) localStorage.setItem('geniusact_user_footprints', JSON.stringify(serverData.geniusact_user_footprints));
        if (serverData.geniusact_global_wallets) localStorage.setItem('geniusact_global_wallets', JSON.stringify(serverData.geniusact_global_wallets));
      }
    }
  } catch(e) {
    console.warn('[Admin] Server fetch failed, falling back to localStorage:', e);
  }

  // FALLBACK: Use localStorage if server fetch failed
  if (!serverData) {
    ...
    visitors = JSON.parse(localStorage.getItem('geniusact_visitor_logs')) || [];
  }
...
// ==================== USER ACTIONS ====================
window.approveUser = async function(identifier) {
  ...
  if (!user && pendingUsers.length > 0 && !isNaN(identifier)) {
    user = pendingUsers[parseInt(identifier)];
  }

  if (user) {
    // SERVER-FIRST: Send approval to server and only update locally on success
    ...
    try {
      ...
      console.warn('[Admin] Server approve error:', err);
    }

    if (!serverSuccess) {
      showToast('⚠️ Server unreachable — approval may not sync to other devices.', 'warning');
    }
    ...
    const pIdx = pendingUsers.findIndex(u => u.email === user.email);
    if (pIdx > -1) pendingUsers.splice(pIdx, 1);

    const aIdx = approvedUsers.findIndex(u => u.email === user.email);
    if (aIdx > -1) approvedUsers[aIdx] = { ...approvedUsers[aIdx], ...user };
    else approvedUsers.push(user);
...
window.rejectUser = async function(identifier) {
  ...
  if (user) {
    try {
      const fetchFn = window.geniusFetch || fetch;
      await fetchFn('/api/admin/reject-user', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ uid: user.uid, email: user.email })
      });
    } catch(err) {
      console.warn('[Admin] Server reject error:', err);
    }

    const filtered = pendingUsers.filter(u => u.email !== user.email && u.uid !== user.uid);
    localStorage.setItem('geniusact_pending_users', JSON.stringify(filtered));
...
if (addFundsBtn) {
  addFundsBtn.addEventListener('click', async () => {
    ...
    const users = JSON.parse(localStorage.getItem('geniusact_approved_users')) || [];
    const user = users.find(u => (u.uid && u.uid === selectedUserId) || (u.email && selectedUserId && u.email.toLowerCase() === selectedUserId.toLowerCase()));

    if (user) {
      try {
        const fetchFn = window.geniusFetch || fetch;
        await fetchFn('/api/admin/add-funds', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ email: user.email, amount, description: desc })
        });
      } catch(err) {
        console.warn('[Admin] Server add-funds error:', err);
      }
      ...
      user.donations.unshift(newFunding);

      const totalAmt = user.donations.reduce((sum, d) => sum + (parseFloat(d.amount) || 0), 0);
      user.amount = totalAmt;
      user.customTotalContributed = totalAmt;
      ...
      if (curStr) {
        ...
        if (cur.email && cur.email.toLowerCase() === user.email.toLowerCase()) {
          ...
          cur.amount = totalAmt;
          cur.customTotalContributed = totalAmt;
          localStorage.setItem('geniusact_current_user', JSON.stringify(cur));
        }
      }

      showToast(`Successfully added $${amount.toLocaleString('en-US', { minimumFractionDigits: 2 })} to ${user.email}`);
      document.getElementById('funds-amount').value = '';
      document.getElementById('funds-desc').value = '';
...
function loadContactUsChats() {
  ...
  supportMsgs.forEach(m => {
    ...
    if (!existing) {
      chats.push({
        chatId: 'user_' + (m.userEmail ? m.userEmail.toLowerCase().replace(/[^a-z0-9]/g, '_') : ('guest_' + Date.now())),
...
function renderAdminChatThread(chatId) {
  ...
  if (headerName) headerName.textContent = `${displayName} (Account ID: ${accIdStr})`;
  if (headerEmail) headerEmail.textContent = `${chat.userEmail || 'Guest Visitor'} • Account ID: ${accIdStr} • Session: ${chat.chatId}`;
  if (headerStatus) {
  ...
  if (msgs.length === 0) {
    threadBody.dataset.renderedHash = 'empty_' + chatId;
    threadBody.innerHTML = `
  ...
  // Check if messages have actually changed before modifying DOM to allow seamless scrolling
  const currentMsgCount = msgs.length;
  const lastMsgId = msgs.length > 0 ? (msgs[msgs.length - 1].id || msgs[msgs.length - 1].timestamp) : '';
  const currentHash = `${chatId}_${currentMsgCount}_${lastMsgId}`;

  const isSwitchedChat = window._lastRenderedChatId !== chatId;
  const isForceScroll = Boolean(window._forceChatScrollBottom);

  if (threadBody.dataset.renderedHash === currentHash && !isSwitchedChat && !isForceScroll) {
    // State is unchanged; do not overwrite innerHTML so user scroll position is completely preserved
    return;
  }

  const prevScrollTop = threadBody.scrollTop;
  const isNearBottom = (threadBody.scrollHeight - threadBody.scrollTop - threadBody.clientHeight) < 80;

  threadBody.dataset.renderedHash = currentHash;
  window._lastRenderedChatId = chatId;
  ...
  if (isSwitchedChat || isForceScroll || isNearBottom) {
    setTimeout(() => {
      if (threadBody) threadBody.scrollTop = threadBody.scrollHeight;
    }, 40);
    window._forceChatScrollBottom = false;
  } else {
    // Preserve exact scroll position if user was reading/scrolling up
    threadBody.scrollTop = prevScrollTop;
  }
}
...
window.saveActiveUserDashboard = async function() {
  ...
  if (user.customTotalContributed !== undefined && !isNaN(user.customTotalContributed)) {
    user.amount = user.customTotalContributed;
    if (!user.donations || user.donations.length === 0) {
      user.donations = [{
        id: 'RCP-' + Math.floor(1000000 + Math.random() * 9000000),
        amount: user.customTotalContributed,
        description: 'Federal Campaign Deposit & Allocation',
        date: new Date().toISOString()
      }];
    }
  }
  ...
  try {
    const fetchFn = window.geniusFetch || fetch;
    await fetchFn('/api/admin/update-user', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ email: user.email, updates: user })
    });
  } catch (err) {
    console.warn('[Admin] Server update-user warning:', err);
  }
...
window.clearAllApplicationData = async function() {
  if (!confirm('Are you sure you want to permanently clear all users, deposits, donations, visitor logs, and messages? This cannot be undone.')) {
    return;
  }
  ...
  try {
    const fetchFn = window.geniusFetch || fetch;
    await fetchFn('/api/admin/clear-all-data', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ token: localStorage.getItem('genius_admin_token') || 'admin' })
    });
  } catch(err) {
    console.error('Error contacting server to clear data:', err);
  }

  showToast('All user data and logs have been completely cleared.');
  setTimeout(() => {
    window.location.reload();
  }, 1000);
};
